...
class ScaledDotProductAttention(nn.Module):
    ...
    def __init__(self, d_model, h, dropout=.1):
        ...
        self.fc_q = nn.Linear(d_model, h * d_model)
        self.fc_k = nn.Linear(d_model, h * d_model)
        self.fc_v = nn.Linear(d_model, h * d_model)
        self.fc_o = nn.Linear(h * d_model, d_model)
...
if __name__ == '__main__':
    ...
    sa = ScaledDotProductAttention(d_model=512, h=8)
    output=sa(input)
    print(output.shape)
