...
# class ParallelEmbedding(nn.Module):  # 由于vocab_size被去除，那这个embedding的方式就应该被修改
#     """
...
# class ColumnParallelLinear(Linear):由于这部分也使用了vocalb_size，所以去除
#     """
...
class MLP(nn.Module):
    ...
    def __init__(self, dim: int, inter_dim: int):
        ...
        self.w1 = nn.Linear(dim, inter_dim)  # 替换 w1
        self.w2 = RowParallelLinear(inter_dim, dim)
        # self.w3 = ColumnParallelLinear(dim, inter_dim)
        self.w3 = nn.Linear(dim, inter_dim)  # 替换 w3
...
class MoE(nn.Module):
    ...
    Attributes:
        ...
        experts (nn.ModuleList): List of expert models.
...
class Transformer(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        # self.embed = ParallelEmbedding(args.vocab_size, args.dim) # 被替换成其他方式的编码
        self.embed = WordEmbedding(args.dim) # 替换成自己写好的WordEmbedding
        self.layers = torch.nn.ModuleList()
        for layer_id in range(args.n_layers):
            self.layers.append(Block(layer_id, args))
        self.norm = RMSNorm(args.dim)
        # self.head = ColumnParallelLinear(args.dim, args.vocab_size, dtype=torch.get_default_dtype())
        self.head = nn.Linear(args.dim, args.vocab_size, dtype=torch.get_default_dtype())  # 替换 head 为 nn.Linear
