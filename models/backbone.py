...
class Backbone(torch.nn.Module):
    def __init__(self, enc_in, config):
        ...
        self.num_nodes = 7 if config.dataset == "101_acc" else 9
        ...
        self.gcn = GnnFamily(
            ...
            order=config.num_layers,
            gcn_method=config.gcn_method,
            norm_method=config.norm_method,
            ffn_method=config.ffn_method,
        )
        self.fc = torch.nn.Linear(config.d_model, 1)
        # self.fc = torch.nn.Linear(config.d_model * 9, 1)

    def forward(self, graph, op_idx):
        op_embeds = (
            self.op_embedding(op_idx).reshape(op_idx.shape[0] * 9, -1)
            + self.device_embedding
        )
        # op_embeds = self.op_embedding(op_idx).reshape(op_idx.shape[0] * op_idx.shape[1], -1)
